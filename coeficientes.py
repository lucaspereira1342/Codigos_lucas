...
pmdec = np.array([-1.748328610744395, -23.980873804807878, 2.049028972886714, -1.9079271011243355, -0.7863207249553673])

print(alfa_final(dec_0, alfa_0, mi_0))

def dec_final (dec, dec_i, m):
    angulo_radiano = np.radians(dec)
    mi_final = (2.8e-6 * m)/np.cos(angulo_radiano)
    dec_f = dec_i + mi_final * (2010.33 - 2000)

    return dec_f

print(dec_final(dec_0, dec_0, pmdec))
